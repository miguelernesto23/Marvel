...
function ModelCard({ ev, onClose }) {
  ...
  const imgSrc = ev.thumbnail
    ? toHttps(`${ev.thumbnail.path}.${ev.thumbnail.extension}`)
    : "/placechorde.jpg";
  ...
  const dateRange = `${formatDate(ev.start)} – ${formatDate(ev.end)}`;

  // Optional featured lists
  const topComics = ev.comics?.items
    ? ev.comics.items
        .slice(0, 8)
        .map((i) => i.name)
        .filter(Boolean)
    : [];
  const topSeries = ev.series?.items
    ? ev.series.items
        .slice(0, 8)
        .map((i) => i.name)
        .filter(Boolean)
    : [];
  const topStories = ev.stories?.items
    ? ev.stories.items
        .slice(0, 8)
        .map((i) => i.name)
        .filter(Boolean)
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Detalles de ${ev.title}`}
      >
        <motion.div
          className="bg-card text-card-foreground relative lg:inline rounded-2xl shadow-2xl max-w-7xl w-full max-h-[85vh] overflow-y-auto no-scrollbar border border-border"
          ...
          <div className="grid md:grid-cols-2">
            ...
            <div className="p-5 md:p-6">
              <div className="flex items-start justify-between gap-3 mb-2">
                <h3 className="text-2xl font-bold leading-tight">{ev.title}</h3>
                <button
                  onClick={onClose}
                  className="lg:inline-flex absolute top-2 z-50   items-center justify-center rounded-full w-9 h-9 border border-border hover:bg-accent hover:text-accent-foreground transition"
                  aria-label="Cerrar"
                >
                  ✕
                </button>
              ...
              <div className="space-y-1">
                <InfoRow label="Rango de fechas" value={dateRange} />
                <InfoRow
                  label="Comics"
                  value={
                    typeof ev.comics?.available === "number"
                      ? ev.comics.available.toLocaleString()
                      : "—"
                  }
                />
                <InfoRow
                  label="Series"
                  value={
                    typeof ev.series?.available === "number"
                      ? ev.series.available.toLocaleString()
                      : "—"
                  }
                />
                <InfoRow
                  label="Historias"
                  value={
                    typeof ev.stories?.available === "number"
                      ? ev.stories.available.toLocaleString()
                      : "—"
                  }
                />
                <InfoRow
                  label="Personajes"
                  value={
                    typeof ev.characters?.available === "number"
                      ? ev.characters.available.toLocaleString()
                      : "—"
                  }
                />
              </div>

              {(topComics.length || topSeries.length || topStories.length) && (
...
/* ---------- Card ---------- */
const EventCard = memo(function EventCard({ ev, onOpen }) {
  const toHttps = (url) =>
    typeof url === "string" ? url.replace(/^http:/, "https:") : url;
      ...
      <div className="p-4">
        ...
        <div className="text-sm flex items-center gap-3 mt-2 text-muted-foreground">
          <span className="flex items-center gap-1">
            <span className="font-semibold text-foreground">
              {ev.characters?.available ?? 0}
            </span>{" "}
            personajes
          </span>
          <span>•</span>
          <span className="flex items-center gap-1">{ev.dateRange ?? "—"}</span>
        </div>
        {ev.description ? (
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            onClick={() => onOpen(ev)}
            aria-label={`Abrir detalles de ${ev.title}`}
          >
            Ver más
          </button>
        </div>
...
export default function Events() {
  ...
  const mappedEvents = useMemo(() => {
    ...
    return rawEvents.map((e) => {
      const thumb = e.thumbnail
        ...
        : "/placechorde.jpg";
      const dateRange = `${formatDate(e.start)} - ${formatDate(e.end)}`;
      return {
        ...e,
        title: e.title,
  ...
  return (
    <main className="min-h-screen bg-background">
        ...
        <div className="container mx-auto">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="text-xl md:text-2xl text-muted-foreground mb-8 max-w-3xl mx-auto">
              Discover the most iconic arcs shaping the Marvel universe.
            </p>
      ...
      <AnimatePresence>
        {selected && (
          <ModelCard ev={selected} onClose={() => setSelected(null)} />
        )}
      </AnimatePresence>
    </main>
  );
